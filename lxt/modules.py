...
class LoraLinearLRP(LoraLinear):
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    result = lf.add2(
                        result, lf.mul2(lora_B(lora_A(dropout(x))), scaling)
                    )
                else:
                    x = dropout(x)
                    ...
                    result = lf.add2(
                        result,
                        self._apply_dora(x, lora_A, lora_B, scaling, active_adapter),
                    )
...
def initialize_adapter(original, replacement):
    """
    Initialize a replacement module with the correct arguments.
    """
    # kwargs = {}
    ...
    # kwargs["adapter_name"] = original._active_adapter[0]
    # replacement = replacement(**kwargs)
    # copy_parameters_and_buffers_(original, replacement)
    import types

    original.forward = types.MethodType(replacement.forward, original)
    return original
...
INIT_MODULE_MAPPING = {
    ...
    LoraLinearLRP: initialize_adapter,
}
