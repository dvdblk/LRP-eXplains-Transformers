...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.gate_proj = lm.LinearEpsilon(
            self.hidden_size, self.intermediate_size, bias=False
        )
        self.up_proj = lm.LinearEpsilon(
            self.hidden_size, self.intermediate_size, bias=False
        )
        self.down_proj = lm.LinearEpsilon(
    ...
    def forward(self, x):
        ...
        return self.down_proj(
            self.proj_silu_mul(self.silu(self.gate_proj(x)), self.up_proj(x))
        )
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.q_proj = lm.LinearEpsilon(
            self.hidden_size, self.num_heads * self.head_dim, bias=config.attention_bias
        )
        self.k_proj = lm.LinearEpsilon(
            self.hidden_size,
            self.num_key_value_heads * self.head_dim,
            bias=config.attention_bias,
        )
        self.v_proj = lm.LinearEpsilon(
            self.hidden_size,
            self.num_key_value_heads * self.head_dim,
            bias=config.attention_bias,
        )
        self.o_proj = lm.LinearEpsilon(
    ...
    def forward(
        ...
        query_states, key_states = apply_rotary_pos_emb(
            query_states, key_states, cos=cos, sin=sin, position_ids=position_ids
        )  # EDITED
        ...
        attn_weights = lf.mul2(
            lf.matmul(query_states, key_states.transpose(2, 3)),
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.input_layernorm = RMSNormIdentity(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = RMSNormIdentity(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, lm.LinearEpsilon):
    ...
    def _prepare_decoder_attention_mask(
        ...
        if attention_mask is not None:
            ...
            combined_attention_mask = (
                ...
                else expanded_attn_mask + combined_attention_mask  ### FLAG
            )
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            output = (logits,) + outputs[1:]  ### FLAG

            return (loss,) + output if loss is not None else output  ### FLAG
